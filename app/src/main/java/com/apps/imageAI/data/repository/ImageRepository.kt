...
class ImageRepositoryImpl @Inject constructor(@ApplicationContext val application: Context, private val ImageAIService: imageAIService, private val apiKeyHelpers: ApiKeyHelpers,private val preferenceRepository: PreferenceRepository) :ImageRepository {

     override  fun generateImageWithDalle(request: ImageRequest): Flow<ImageGenerationStatus> = callbackFlow {

          var downloadId:Int?=null
          runCatching {
               val result = ImageAIService.generateImages(request,"Bearer ${apiKeyHelpers.getApiKey()}")
               if (result.data!=null)
               {
     ...
     override fun generateImageWithStability(request: StabilityImageRequest): Flow<ImageGenerationStatus> = callbackFlow {
          ...
          runCatching {
               ...
               val result =    ImageAIService.generateImagesWithStability("stable-diffusion-xl-1024-v1-0", body = request, authHeader = "Bearer ${apiKeyHelpers.getStabilityKey()}")
    ...
    private suspend fun getENText(text: String):String
    {
         ...
         if (isTemplate.not()){
              // api call
              runCatching {
                   val result = ImageAIService.translateText(preferenceRepository.getCurrentLanguageCode(),"en",requestText)
